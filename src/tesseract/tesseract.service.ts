...
export class TesseractService {
  ...
  public async recognizeTextFromImage (
    ...
    if (uploadedImages !== null && uploadedImages.length > 0) {
      ...
      for (const eachImage of uploadedImages) {
        ...
        const resizedImage = await this.convertImage(eachImage.path, targetLocation);
        const recognizedText = await this.recognize(resizedImage.resizedImagePath, RECOGNIZE_OCR_LANGUAGE, resizedImage.locatedRectangle);
  ...
  public async recognize(imgPath: string, lang: string, locatedRectangle: any): Promise<string> {
    ...
    /*
      You can use this below to set whitelist in language:
